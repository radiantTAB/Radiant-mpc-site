...
visual theme but is **not part of the radiant-mpc.com site** (it's excluded
from the served assets via `.assetsignore`).
...
Repo `radiantTAB/radiant-mpc-site`, branch **`claude/personal-income-database-yepmbn`**.

```powershell
cd C:\radiant-mpc-site          # your existing clone (see repo README)
git fetch origin
git checkout claude/personal-income-database-yepmbn
git pull origin claude/personal-income-database-yepmbn
```

All the work lives in **`personal-income-tracker/`**:
...
To run it: **just double-click `index.html`** (or `Start-Process .\personal-income-tracker\index.html`). No build, no server.
...
Stay on the branch and keep committing there:

```powershell
git add personal-income-tracker/index.html
git commit -m "…"
git push origin claude/personal-income-database-yepmbn
```

Do **not** commit any `.json` that contains real income data. The app +
docs are safe to commit; the data backups are not.
