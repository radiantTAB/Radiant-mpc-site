...
-- ============================================================
-- Income Tracker (personal, admin-only). Created on demand by
-- ensureIncomeSchema() in worker/income.js, so a fresh D1 needs no
-- manual step -- these are here for reference/documentation.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS income_rates (
  ...
  rate           REAL NOT NULL,
  effective_date TEXT NOT NULL,   -- ISO date the rate takes effect
  created_at     TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS income_weeks (
  id          TEXT PRIMARY KEY,
  site_id     TEXT NOT NULL,
  week_ending TEXT NOT NULL,      -- ISO date (week-ending anchor)
